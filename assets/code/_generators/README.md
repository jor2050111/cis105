...
| `generate_chapter10_data.py` | `assets/code/chapter-10/saguaro-bookings.csv` and `saguaro-customers.csv` | 60 x 13 bookings plus a 24 x 6 customer dimension: referential integrity for a clean Airtable link, 17 repeat organizations, one planted phone drift (SH-1005), a reproducible pricing model, two capacity overflows, byte-identical on rerun. Standard library only (no openpyxl) |
| `generate_chapter11_data.py` | `assets/code/chapter-11/project-tasks.csv` | 54 x 11 tasks for Saguaro Hall's booking-system rollout across the six life-cycle phases (nine each): all four statuses, valid dependencies, one at-risk task (T-140 In Progress ahead of its unstarted prerequisite T-136), 11 overdue tasks on the 2026-07-11 status date, byte-identical on rerun. Standard library only |
| `generate_chapter12_data.py` | `assets/code/chapter-12/booking-export.xlsx` and `ai-macro-draft.txt` | 63 x 10 October 2026 booking export: Chapter 10 pricing model and org cast (imported, not copied), the base's four October bookings inherited verbatim, no space double-booked per date, dates ascending, correct totals 63/$83,910 versus the AI draft's stale-range 60/$78,510 (a $5,400 miss pinned to the last three rows), byte-identical on rerun. Needs openpyxl |
